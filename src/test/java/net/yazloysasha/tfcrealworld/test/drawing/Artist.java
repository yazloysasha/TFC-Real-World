...
public abstract class Artist<T, A extends Artist<T, A>> {
  ...
  public static <K, V> Artist.Typed<K, V> forMap(
    Function<K, Pixel<V>> transformer
  ) {
    return new Artist.Typed<>(transformer); // An artist that handles pixel -> K objects
  }

  public static <V> Artist.Noise<V> forNoise(
    Function<V, NoisePixel> transformer
  ) {
    return new Artist.Noise<>(transformer); // An artist that handles pixel -> double objects
  }

  public static <V> Artist.Colored<V> forColor(
    Function<V, Pixel<Color>> transformer
  ) {
  ...
  protected A copy(Artist<?, ?> other) {
    dimensions(other.minX, other.minY, other.maxX, other.maxY).size(
      other.width
    );
    return (A) this;
  }
  ...
  protected final void drawStream(
    Graphics graphics,
    Stream<Local<Color>> stream
  ) {
    stream.forEach(loc -> {
  ...
  public static final class Scales {
    ...
    public static final ScaleTransformer NEAREST_INT = (value, min, max) ->
      (int) (value + 0.5);
    public static final ScaleTransformer DYNAMIC_RANGE = (value, min, max) ->
      max == min ? min : (value - min) / (max - min);

    public static ScaleTransformer fixedRange(
      double fixedMin,
      double fixedMax
    ) {
      return (value, min, max) ->
        DYNAMIC_RANGE.apply(value, fixedMin, fixedMax);
    }
  ...
  public static final class Noise<T> extends Artist<T, Noise<T>> {
    ...
    protected void drawInternal(String name, T instance, Graphics graphics) {
      final double[] sourceMinMax = new double[] {
        Double.MAX_VALUE,
        Double.MIN_VALUE,
      };
      ...
      drawStream(
        ...
        stream((x, y) -> {
          ...
          .toList()
          .stream() // Block after min/max calculations
          .peek(loc -> {
            if (histogram) {
              final double scaled = Scales.DYNAMIC_RANGE.apply(
                loc.value,
                sourceMinMax[0],
                sourceMinMax[1]
              );
          ...
          .map(
            Local.map(value ->
              color.apply(
                scaleTransformer.apply(value, sourceMinMax[0], sourceMinMax[1])
              )
            )
          )
      );
  ...
  public static final class Typed<K, V> extends Artist<K, Typed<K, V>> {
    ...
    public Noise<K> mapNoise(ToDoubleFunction<V> noiseTransformer) {
      return Artist.<K>forNoise(
        v ->
          (x, y) ->
            noiseTransformer.applyAsDouble(transformer.apply(v).apply(x, y))
      ).copy(this);
    }

    public <R> Typed<K, R> mapTo(Function<V, R> transformer) {
      return Artist.<K, R>forMap(
        k -> (x, y) -> transformer.apply(this.transformer.apply(k).apply(x, y))
      ).copy(this);
    }
    ...
    protected void drawInternal(String name, K instance, Graphics graphics) {
      ...
      drawStream(
        graphics,
        stream((x, y) -> colorTransformer.apply(pixel.apply(x, y)))
      );
    }
  }
  ...
  private record Local<T>(int x, int y, T value) {
    public static <A, B> Function<Local<A>, Local<B>> map(
