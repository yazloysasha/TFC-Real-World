...
public class RegionGeneratorTest extends TestHelper {
  ...
  private void drawStitchedRegions(
    String name,
    Set<RegionGenerator.Task> tasksToDraw,
    int size
  ) {
    ...
    for (int x = 0; x < size; x++) for (int z = 0; z < size; z++) points.add(
      new Pos(x, z)
    );
    ...
    while ((pos = points.stream().findFirst().orElse(null)) != null) {
      generator.visualizeRegion(pos.x, pos.z, (task, region) -> {
        ...
        final Map<Pos, Color> drawnTask = drawn.computeIfAbsent(task, key ->
          new HashMap<>()
        );
        for (int rx = region.minX(); rx <= region.maxX(); rx++) for (
          int rz = region.minZ();
          rz <= region.maxZ();
          rz++
    ...
    drawn.forEach((task, drawnTask) ->
      Artist.raw()
        .dimensions(size)
        .size(size * (task == ADD_RIVERS_AND_LAKES ? 8 : 1))
        .draw(
          taskName(name, task),
          task == ADD_RIVERS_AND_LAKES
  ...
  private Artist.Pixel<Color> drawWithRivers(
    ...
    return (xi, zi) -> {
      ...
      final RegionPartition.Point point = generator.getOrCreatePartitionPoint(
        x,
        z
      );
  ...
  private Color taskColor(
    ...
    if (!point.land()) {
      return switch (task) {
        case ANNOTATE_BASE_LAND_HEIGHT -> point.baseOceanDepth < 4
          ? new Color(150, 160, 255)
          : point.baseOceanDepth < 8
            ? new Color(120, 120, 240)
            : new Color(100, 100, 200);
        case ANNOTATE_CLIMATE -> blue.apply(
          Mth.clampedMap(point.temperature, -35f, 35f, 0f, 0.999f)
        );
        case ANNOTATE_RAINFALL -> blue.apply(
          Mth.clampedMap(point.rainfall, 0f, 500f, 0f, 0.999f)
        );
        default -> point.shore()
          ? (point.river()
              ? new Color(150, 160, 255)
    ...
    return switch (task) {
      ...
      case ANNOTATE_DISTANCE_TO_OCEAN -> green.apply(
        point.distanceToOcean / 20f
      );
      case ADD_RIVERS_AND_LAKES -> point.lake()
        ? new Color(150, 160, 255)
        : green.apply(point.baseLandHeight / 24f);
      case ANNOTATE_BASE_LAND_HEIGHT -> green.apply(point.baseLandHeight / 24f);
      case ANNOTATE_BIOME_ALTITUDE -> green.apply(
        Mth.clampedMap(point.discreteBiomeAltitude(), 0, 3, 0, 1)
      );
      case ANNOTATE_CLIMATE -> temperature.apply(
        Mth.clampedMap(point.temperature, -35f, 35f, 0f, 0.999f)
      );
      case ANNOTATE_RAINFALL -> temperature.apply(
        Mth.clampedMap(point.rainfall, 0f, 500f, 0f, 0.999f)
      );
    };
  }
  ...
  private RegionGenerator newRegionGenerator() {
    return new RegionGenerator(
      new Settings(false, 0, 0, 0, 20_000, 0, 20_000, 0, null, 0.5f, 0.5f),
      new XoroshiroRandomSource(1798237841231L)
    );
  }
}
