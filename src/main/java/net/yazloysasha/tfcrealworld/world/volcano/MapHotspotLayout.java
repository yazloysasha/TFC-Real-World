...
/**
 * Scans the hotspot PNG into discrete volcanic centers and resolves mountain
 * style (shield / stratovolcano / natural) for {@link MapHotspotBiomes}.
 */
public final class MapHotspotLayout {
  ...
  private static final double INV_GRID_WIDTH_IN_BLOCK =
    1.0 / TFCRealWorld.GRID_WIDTH_IN_BLOCK;
  private static final double MIN_RADIUS_BLOCKS = 384;
  private static final double BIOME_RADIUS_SCALE = 1.05;
  private static final double HALF_GRID_BLOCK =
    TFCRealWorld.GRID_WIDTH_IN_BLOCK * 0.5;
  ...
  public Noise2D intensityNoise(byte age, long seed) {
    ...
    final OpenSimplex2D warp = new OpenSimplex2D(seed + 7919L * age)
      ...
      .spread(0.004f)
      .scaled(-0.1f, 0.1f);
    return (x, z) -> (float) peakAt(x, z, scoped, warp.noise(x, z));
  }
