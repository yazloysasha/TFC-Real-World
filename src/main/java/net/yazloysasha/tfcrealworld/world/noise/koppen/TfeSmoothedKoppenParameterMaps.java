...
/**
 * TFE copy of 1.21.1 SmoothedKoppenParameterMaps (temperature, rainfall, rainVar).
 * Enforces local neighbor limits in map-grid space.
 */
public final class TfeSmoothedKoppenParameterMaps {
  ...
  private static volatile TfeSmoothedKoppenParameterMaps instance;
  ...
  private TfeSmoothedKoppenParameterMaps(
    String profileId,
    int width,
    int height,
    int[] packedIdx
  ...
  public static TfeSmoothedKoppenParameterMaps getInstance() {
    String profileId = TFCRealWorldConfig.MAP_PROFILE.get();

    TfeSmoothedKoppenParameterMaps cached = instance;
    if (cached != null && cached.profileId.equals(profileId)) {
      return cached;
    }

    synchronized (TfeSmoothedKoppenParameterMaps.class) {
      cached = instance;
      if (cached != null && cached.profileId.equals(profileId)) {
        return cached;
      }
  ...
  private static TfeSmoothedKoppenParameterMaps build(String profileId) {
    ...
    TfeKoppenParameterCache cache = TfeKoppenParameterCache.getInstance();
    ...
    for (int i = 0; i < size; i++) {
      ...
      TfeKoppenParameterCache.ParameterCombination params =
    ...
    return new TfeSmoothedKoppenParameterMaps(
      profileId,
      width,
      height,
      packedIdx
