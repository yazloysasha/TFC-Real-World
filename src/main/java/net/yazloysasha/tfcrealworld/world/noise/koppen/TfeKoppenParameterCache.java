...
import net.yazloysasha.tfcrealworld.world.climate.TfeKoppenClassification;

/**
 * TFE copy of 1.21.1 KoppenParameterCache: temp/rain/rainVar combinations from TfeKoppenClassification.
 */
public class TfeKoppenParameterCache {
  ...
  private static TfeKoppenParameterCache instance;
  private final Map<
  ...
  private final Map<RealKoppenClimateClassification, float[]> rainVarRanges;
  private final Map<
    RealKoppenClimateClassification,
    ParameterGrid
  > parameterGrids;

  private TfeKoppenParameterCache() {
  ...
  public static synchronized TfeKoppenParameterCache getInstance() {
    if (instance == null) {
      instance = new TfeKoppenParameterCache();
    }
    return instance;
  }
  ...
  public float[] getRainVarRange(RealKoppenClimateClassification climate) {
    ...
    for (float temp : temperatures) {
      for (float rain : rainfalls) {
        for (float rainVar : rainVars) {
          RealKoppenClimateClassification climate =
            TfeKoppenClassification.classify(temp, rain, rainVar, true);
